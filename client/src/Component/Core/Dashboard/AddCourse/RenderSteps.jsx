    ...
    <div>
      ...
      <div className='flex w-full justify-center mb-2' >
        {
          steps.map((item) => (
            <Fragment key={item.id}>
              <div className={`grid place-items-center aspect-square rounded-full w-8.5 border select-none
...

              {
                item.id !== steps.length && (
                  <>
                    <div className={`h-4.25 w-[33%] border-b-2 border-dashed
      ...
      <div className='mb-10 md:mb-16'>
        <div className='hidden md:flex justify-between select-none ' >
          {
            steps.map(item => (
              <div key={item.id} className={`min-w-32.5 text-center text-sm  uppercase tracking-wider
